...
colnames(p.driver.wide) = c("time","SERC_p","HARV_p","UNDE_p","GRSM_p","NA","SERC_light","HARV_light","UNDE_light","GRSM_light",
                            "SERC_temp","HARV_temp","UNDE_temp","GRSM_temp","SERC_precip","HARV_precip","UNDE_precip","GRSM_precip")
...
m_rwdse   <- coda.samples(model = j.model.rwdse,
                      variable.names = c("sd_add","sd_site","site","sd_obs","betaDriver","x","y"),
                      n.iter = 3000,
                      thin = 1)

plot(m_rwdse[,c(1,2,804)])
...
ggplot(data=model_ci.dse,aes(x = time, y = mean.x)) +
        ...
        facet_wrap(~as.factor(site))+
        theme_bw()
list = c(site_mdse,sd_mdse,beta_mdse))
...
for (i in 1:4){
I_forecast_rwse[[i]] = forecastP.RWSRE(IC=pred_rwse[prow,801*i],
                  site=mean(site_rwse[,i]),
                  Q=0,
                  n=Nmc,
                  NT=34)
IP_forecast_rwse[[i]] = forecastP.RWSRE(IC=pred_rwse[prow,801*i],
                  site=site_rwse[prow,i],
                  Q=0,
                  n=Nmc,
                  NT=34)
IPP_forecast_rwse[[i]] = forecastP.RWSRE(IC=pred_rwse[prow,801*i],
                  site=site_rwse[prow,i],
                  Q=sd_rwse[prow,1],
                  n=Nmc,
                  NT=34)
siteNew.mc <- rnorm(Nmc,0,sd_rwse[,803])
IPPR_forecast_rwse[[i]] = forecastP.RWSRE(IC=pred_rwse[prow,801*i],
                  site=siteNew.mc,
                  Q=sd_rwse[prow,1],
                  n=Nmc,
                  NT=34)
...
ggplot(data=IPPR_forecast_rwse[[i]][[2]],aes(x=date,y=forecast_gcc_med)) +
  ...
  theme_classic()
  ggsave(paste0("Uncert_Propogating_RandomWalkRandomEffect_",unique(future_sun_ensembles$site_id)[i],"_figure.pdf"), device = "pdf")
}
...
## Random walk, driver, and site random effect forecast
```{r}
...
for (i in 1:4){
I_forecast[[i]] =forecastP.RWTDSRE(IC=pred_rwdse[prow,801*i],
...
IP_forecast[[i]] = forecastP.RWTDSRE(IC=pred_rwdse[prow,801*i],
...
future_sun_ensembles_site =future_sun_ensembles[future_sun_ensembles$site_id == unique(future_sun_ensembles$site_id)[i],]
## Randomly subsample
drow = sample.int(nrow(future_sun_ensembles_site[1:30,]),Nmc,replace=TRUE)

##Forecast
IPD_forecast[[i]] = forecastP.RWTDSRE(IC=pred_rwdse[prow,801*i],
                  site=site_rwdse[prow,i],
                  Q=0,
                  n=Nmc,
...
IPDP_forecast[[i]] = forecastP.RWTDSRE(IC=pred_rwdse[prow,801*i],
                  site=site_rwdse[prow,i],
                  Q=sd_rwdse[prow,1],
                  n=Nmc,
                  Driver = future_sun_ensembles_site[drow,4:37],
...
siteNew.mc <- rnorm(Nmc,0,sd_rwdse[,803])

IPDPR_forecast[[i]] = forecastP.RWTDSRE(IC=pred_rwdse[prow,801*i],
                  site=siteNew.mc,
                  Q=sd_rwdse[prow,1],
                  n=Nmc,
                  Driver = future_sun_ensembles_site[drow,4:37],
...
ggplot(data=IPDPR_forecast[[i]][[2]],aes(x=date,y=forecast_gcc_med)) +
  ...
  geom_line(aes(x=date,y=forecast_gcc_med))+
  theme_classic()
  ggsave(paste0("Uncert_Propogating_RandomWalkDriverRandomEffect_",unique(future_sun_ensembles$site_id)[i],"_figure.pdf"), device = "pdf")
}
...
ggplot(data=var.rel[[i]],aes(x=date, y=IPDPR.var))+
    ...
    geom_ribbon(data=var.rel[[i]],aes(ymin = rep(0,34), ymax = I.var), 
                    color = "light blue", 
                    fill = "light blue")
    ggsave(paste0("Uncert_Partitioning_RandomWalkDriverRandomEffect_",unique(future_sun_ensembles$site_id)[i],"_figure.pdf"), device = "pdf")

}

```

## Forecast for driver and random effect model
```{r}
I_forecast_dse = list()
IP_forecast_dse = list()
IPP_forecast_dse = list()
IPPR_forecast_dse = list()
for(i in 1:4){
I_forecast_dse[[i]] =forecastP.DSE(IC=pred_mdse[prow,801*i],
                  ...
                  NT=34)
IP_forecast_dse[[i]] = forecastP.DSE(IC=pred_mdse[prow,801*i],
...
IPD_forecast_dse[[i]] = forecastP.DSE(IC=pred_mdse[prow,801*i],
                  site=site_mdse[prow,i],
                  Q=0,
                  n=Nmc,
                  Driver = future_sun_ensembles_site[drow,4:37],
...
IPDP_forecast_dse[[i]] = forecastP.DSE(IC=pred_mdse[prow,801*i],
                  site=site_mdse[prow,i],
                  Q=sd_mdse[prow,1],
                  n=Nmc,
                  Driver = future_sun_ensembles_site[drow,4:37],
...
se_new_mdse <- rnorm(Nmc,0,sd_mdse[,803])
IPDPR_forecast_dse[[i]] = forecastP.DSE(IC=pred_mdse[prow,801*i],
                  site=se_new_mdse,
                  Q=sd_mdse[prow,1],
                  n=Nmc,
                  Driver = future_sunGRSM[drow,4:37],
                  betaDriver = beta_mdse[prow,1],
                  NT=34)
}
...
ggplot(data=IPDP_forecast_dse[[i]][[2]],aes(x=date,y=forecast_gcc_med)) +
  ...
  geom_ribbon(data=I_forecast_dse[[i]][[2]], 
              aes(ymin = forecast_gcc_low, ymax = forecast_gcc_high), 
                    alpha = 1, color = "light blue", 
                    fill = "light blue")+
  geom_line(aes(x=date,y=forecast_gcc_med))+
  theme_classic()
  ggsave(paste0("Uncert_Propogating_DriverRandomEffect_",unique(future_sun_ensembles$site_id)[i],"_figure.pdf"), device = "pdf")
}
...
ggplot(data=var.rel.dse[[i]],aes(x=date, y=IPDPR.var))+
    ...
    geom_ribbon(data=var.rel.dse[[i]],aes(ymin = rep(0,34), ymax = I.var), 
                    color = "light blue", 
                    fill = "light blue")
    ggsave(paste0("Uncert_Partitioning_DriverRandomEffect_",unique(future_sun_ensembles$site_id)[i],"_figure.pdf"), device = "pdf")
