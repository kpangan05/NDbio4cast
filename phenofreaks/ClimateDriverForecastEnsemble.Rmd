---
title: "ClimateDriverForecastEnsemble"
author: "Phenofreaks"
date: "11/28/2022"
...
NeonPheno =readr::read_csv("https://data.ecoforecast.org/neon4cast-targets/phenology/phenology-targets.csv.gz", guess_max = 1e6) |> 
  dplyr::filter(datetime >= '2020-09-25')
  na.omit()
head(NeonPheno)
```
...
library(dplyr)
library(tidyr)
sites <- unique(site_data$field_site_id)
...
Pheno_seasons %>% 
  ...
  theme(strip.background=element_blank(),panel.spacing = unit(1,"lines"))
```
## Read in climate driver data
```{r}
library(neon4cast)
df_past <- neon4cast::noaa_stage3()
df_future <- neon4cast::noaa_stage2()
sites <- c('HARV','UNDE','SERC','GRSM')
#getting past temperature
past_temp <- df_past |>
...
past_temp1 <- past_temp %>% group_by(date = (datetime)) %>%
  summarise(prediction = mean(prediction))

past_temp1 <- past_temp1 %>% group_by(date = (date)) %>%
  summarise(prediction = mean(prediction))

daily_temp$date <- as.Date(past_temp1$date)
daily_temp <- aggregate(daily_temp["prediction"], by=daily_temp["date"], mean)
temp <- daily_temp$prediction
temp <- temp[1:795]
...
model{
  ...
  #added beta priors
  b0 ~ dnorm(0, 0.01) # intercept
	b1 ~ dnorm(0, 0.01) # slope
  
  # Process Model
  for(t in 2:N){
  #added mu value, changed how y is distributed to be around mu instead of random walk
  mu[t] <- b0 + b1*temp[t] 
...
## the following code produces a hindcast of the 5 sites using the above model
```{r}
library(ggplot2)
phenoDat = Pheno.uncert
phenoDat = phenoDat[phenoDat$site_id %in% c('HARV','UNDE','SERC','GRSM'),]
